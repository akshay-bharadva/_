      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-10">
          ...
          <div>
            ...
            {item.subtitle && (
              <p className="text-base font-medium text-primary/80 mb-6">
                {item.subtitle}
              </p>
            )}
            ...
            <div className="space-y-6">
              {PROCESS_STEPS.map(({ step, label, color }) => (
                <div key={step} className="flex gap-4">
                  <div
                    className={`shrink-0 font-mono text-[10px] font-semibold px-2 py-1 rounded-md h-fit mt-0.5 ${color}`}
                  >
                    {step}
                  </div>
                  <div>
                    <div className="text-xs font-semibold text-muted-foreground uppercase tracking-widest font-mono mb-1">
                      {label}
                    </div>
                    {step === "01" && item.description && (
                      <ItemMarkdown>{item.description}</ItemMarkdown>
                    )}
          ...
          <div className="space-y-4">
            <div className="text-xs font-semibold text-muted-foreground uppercase tracking-widest font-mono mb-3">
              Outcomes
            </div>
            {item.tags &&
              item.tags.map((tag, i) => {
                ...
                return (
                  ...
                  >
                    <div className="text-3xl font-black tracking-tighter text-foreground mb-0.5">
                      {value}
                    </div>
