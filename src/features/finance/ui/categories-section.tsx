...
import type { FinBucket, FinCategory } from "@/types";
import {
...
import { FINANCE_LIMITS, finCategorySchema } from "@/lib/schemas";

/**
 * Categories, and the two fields that make the coaching work.
 *
 * `bucket` decides the 50/30/20 split and `is_essential` decides the runway, and
 ...
 * Folding them into one field forces a wrong answer to one of the two questions.
 *
 * Two things this fixes rather than ports:
 *
 * - **Names are validated, not merely truncated.** v1 capped the input with a
 *   `maxLength` attribute and sent whatever came out. An attribute is a courtesy
 *   to someone typing, not a check — it does nothing for a value arriving any
 *   other way — so every write here goes through `finCategorySchema` first.
 * - **A duplicate name is refused here, by name.** `fin_category` has
 *   `UNIQUE (user_id, name)`, so in v1 a second "Groceries" came back as an
 *   opaque failed save with nothing to act on.
 */

const BUCKETS: { id: FinBucket; label: string; hint: string }[] = [
...
export function CategoriesSection({
  ...
  const [deleteCategory] = useDeleteFinCategoryMutation();
  const confirm = useConfirm();

  const [newName, setNewName] = useState("");
  const [newBucket, setNewBucket] = useState<FinBucket>("want");

  const active = categories.filter((category) => !category.archived_at);

  /** Case-insensitively, because "Groceries" and "groceries" are one category. */
  ...
  const rename = async (category: FinCategory, name: string) => {
    if (nameTaken(name, category.id)) {
      toast.error(`You already have a category called ${name.trim()}`);
      return;
    }
    ...
    if (!checked.success) {
      toast.error(checked.error.issues[0]?.message ?? "That name will not do");
      return;
    }
    await patch(category, { name: checked.data.name });
  };
  ...
  const add = async () => {
    ...
    const checked = finCategorySchema.safeParse({
      name,
      bucket: newBucket,
      // A need is essential by default, which is right far more often than not
      // and is one switch away from corrected.
    ...
    try {
      await saveCategory({
        ...checked.data,
        // Spaced by ten so one can be dropped between two later without
  ...
  const remove = async (category: FinCategory) => {
    const ok = await confirm({
      title: `Delete ${category.name}?`,
      description:
        "Postings filed under it keep their amounts but lose the label, so they move into the uncategorised total until you re-file them.",
      confirmText: "Delete",
      variant: "destructive",
    });
    if (!ok) return;
    ...
    <section className="space-y-4" aria-label="Categories">
      <div>
        ...
        {/*
          ...
          Sorting your own categories is tedious, so the screen has to answer
          "and then what?" before anybody will do it.
        */}
      ...
      {active.length === 0 ? (
        <EmptyState
          ...
          description="Add the ones money actually goes to — rent, groceries, transport — and sort each into a bucket as you go. An import will offer to create any it recognises."
        />
      ) : (
        <ul className="space-y-1.5">
          {active.map((category) => (
            ...
            >
              <InlineName
                category={category}
                onRename={(name) => void rename(category, name)}
              />
