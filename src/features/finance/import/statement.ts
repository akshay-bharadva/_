import { fromDecimal, type Money } from "../money/minor-units";

/**
 * What a bank's CSV means.
 *
 * Neither CIBC nor RBC documents its export, so the formats here are recognised
 * by **shape** rather than trusted by name, and anything unrecognised falls back
 * to a column mapping the owner confirms. Every import also shows a sign check
 * before anything is written: a file read with its signs backwards turns a year
 * of groceries into a year of income, and that is a mistake worth a screen of
 * its own.
 *
 ...
 *
 * ## Why a statement row holds a decimal, in a module built on integers
 *
 * `StatementRow.amount` is a decimal number, and that is deliberate rather than
 * an oversight. A statement row is **what the file said** — not a ledger
 * amount. Two things depend on keeping it that way:
 *
 * 1. The import fingerprint hashes `amount.toFixed(2)`. Migration 028 carries
 *    every existing `import_hash` across verbatim, so changing what is hashed
 *    would make every previously imported row look new, and re-importing an
 *    overlapping export would duplicate a year of transactions.
 * 2. The sign heuristics below reason about the signs *as the file wrote them*.
 *
 * Conversion to minor units happens once, at the posting boundary, through
 * `toMinor` — and from the string, never from the float.
 */
...
/** A real calendar date, or null. Rejects 30 February rather than rolling it. */
...
export function parseDate(value: string, order: DateOrder): string | null {
  ...
  // "Sep 15, 2023"
  const words = /^([A-Za-z]{3})[a-z]*\.? (\d{1,2}),? (\d{4})$/.exec(text);
  if (words) {
    const month = MONTHS[words[1].toLowerCase()];
    return month ? valid(+words[3], month, +words[2]) : null;
  }

  // "15 Sep 2023", "15-Sep-2023"
  const dayFirst = /^(\d{1,2})[ -]([A-Za-z]{3})[a-z]*\.?[ -](\d{2,4})$/.exec(
    text,
  );
  if (dayFirst) {
    ...
    let y = +dayFirst[3];
    if (y < 100) y += 2000;
    return month ? valid(y, month, +dayFirst[1]) : null;
  }
...
/**
 * Which way round a column of slash dates is.
 *
 * A first part over 12 settles it as day-first, a second part over 12 as
 * month-first. With neither — a file whose every date is ambiguous — the
 * Canadian banks' own M/D/Y, because that is what the exports this reads
 * actually use.
 */
...
/**
 * `"$1,234.56"`, `"-12.00"`, `"(12.00)"`, `"12.00-"`, `"CAD 40"` — or null
 * when the cell is empty or not a number.
 *
 * Accounting notation has three ways of writing a negative and banks use all
 * of them, sometimes in one file.
 */
...
/**
 * A statement row as money, in the account's currency.
 *
 * Goes through the *string* rather than the parsed float, so `12.345` is read
 * digit by digit rather than from its binary approximation — the same rule the
 * money module enforces everywhere else.
 */
export function toMinor(row: StatementRow, currency: string): Money {
  return fromDecimal(row.amount.toFixed(4), currency);
}
...
export function detectFormat(rows: string[][]): DetectedFormat {
  ...
  if (header.includes("transaction date") && header.includes("description 1")) {
    ...
    const dates = rows
      .slice(1, 40)
      .map((row) => row[at("transaction date")] ?? "");
    ...
    return {
      ...
      columns: {
        ...
        description2:
          at("description 2") === -1 ? undefined : at("description 2"),
        amount: at("cad$") === -1 ? undefined : at("cad$"),
        usdAmount: at("usd$") === -1 ? undefined : at("usd$"),
        accountRef:
          at("account number") === -1 ? undefined : at("account number"),
  ...
  if (
    ...
    width <= 5 &&
    early.every(
      (row) => isAmountLike(row[2] ?? "") && isAmountLike(row[3] ?? ""),
    )
  ) {
    const card = early.some((row) =>
      /\d{4}\*+\d{2,4}|\*{4}\d{4}/.test(row[4] ?? ""),
    );
  ...
  // Anything else: mapped by header names when there is a header, by shape
  // when there is not. Either way the owner sees the mapping and can change it.
  ...
  if (hasHeader) {
    ...
    const paired = debit !== undefined && credit !== undefined;
    ...
    return {
      ...
      columns: {
        ...
        amount: paired ? undefined : (amount ?? 2),
        debit: paired ? debit : undefined,
        credit: paired ? credit : undefined,
        dateOrder: guessDateOrder(body.map((row) => row[date] ?? "")),
      },
    };
  }

  // No header at all: the date column is the one that looks like dates, the
  // description is the wordiest, and the numeric columns are the amounts.
...
export function readRows(
  ...
  for (let i = start; i < rows.length; i += 1) {
    ...
    if (!date) {
      skipped.push({
        line,
        reason: `No date in “${cell(columns.date) || "(empty)"}”`,
      });
      continue;
    }
    ...
    if (columns.debit !== undefined || columns.credit !== undefined) {
      const paidOut = parseAmount(cell(columns.debit));
      const paidIn = parseAmount(cell(columns.credit));
      if (paidOut !== null || paidIn !== null) {
        amount = (paidIn ?? 0) - Math.abs(paidOut ?? 0);
        // A credit column that is itself negative means the bank has already
        // signed it; trust the sign rather than adding it twice.
        if (paidIn !== null && paidIn < 0)
          amount = paidIn - Math.abs(paidOut ?? 0);
      }
    } else {
    ...
    out.push({
      ...
      accountRef:
        columns.accountRef === undefined
          ? null
          : cell(columns.accountRef) || null,
      currency,
    });
  }
...
export function accountRefsIn(
  rows: StatementRow[],
): { ref: string; count: number }[] {
  const counts = new Map<string, number>();
  for (const row of rows) {
    if (row.accountRef) {
      counts.set(row.accountRef, (counts.get(row.accountRef) ?? 0) + 1);
    }
  }
...
const CARD_PAYMENT_IN =
  /PAYMENT THANK YOU|PAIEMENT MERCI|PAYMENT - THANK YOU|PAYMENT RECEIVED/i;
const PAYROLL = /PAYROLL|PAY ?DEP|DIRECT DEP|DIR DEP|SALARY|\bPAY\b/i;

/**
 * Whether the signs look backwards, and why.
 *
 * Only a suggestion — the sign check shows the rows and the owner decides — but
 * it catches the common case before anything is written: a card whose payments
 * read as money out, or a chequing account whose salary does. Reading a file
 * with its signs reversed turns a year of groceries into a year of income, and
 * every figure downstream inherits it.
 */
export function suggestFlip(
  ...
  if (isCard && payments.length > 0) {
    ...
    return backwards > payments.length / 2
      ? {
          flip: true,
          reason:
            "Card payments read as money out, so the signs look backwards.",
        }
  ...
  if (!isCard && pay.length > 0) {
    ...
    return backwards > pay.length / 2
      ? {
          flip: true,
          reason: "Pay reads as money out, so the signs look backwards.",
        }
      : { flip: false, reason: null };
  }
  ...
  if (rows.length >= 10 && positive / rows.length > 0.8) {
    return {
      flip: true,
      reason:
        "Almost everything reads as money in, which is unusual for a statement.",
    };
  }
