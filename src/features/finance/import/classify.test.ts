...
import v1 from "./v1-behaviour.json";
import {
  classify,
  historyFrom,
  isOwnName,
 ...
 *
 * So this compares the copy against **what v1 actually produced**, across every
 * branch of the classifier. If the copy ever diverges — a stray edit, a "tidy" —
 * these fail with the exact input that separated them.
 *
 * `v1-behaviour.json` was generated by running v1's own `classify`,
 * `normaliseMerchant`, `stripChannel` and `historyFrom` over the fixtures below,
 * on the commit that deleted them. Until then these tests called v1 directly and
 * compared; that is better while both exist and impossible once one does not, so
 * the output was frozen rather than the comparison dropped. **Nothing may
 * regenerate this file.** Its whole value is that it predates the deletion — a
 * fixture refreshed from the code it is checking asserts only that the code
 * equals itself.
 */

/** One case's key in the fixture, built the same way the dump built it. */
const caseKey = (
  description: string,
  amount: number,
  accountKind: string,
  detail: string,
) => `${description}|${amount}|${accountKind}|${detail}`;

const classifyCase = (key: string) => {
  const found = (v1.classify as { key: string; result: unknown }[]).find(
    (entry) => entry.key === key,
  );
  if (!found) throw new Error(`No recorded v1 behaviour for ${key}`);
  return found.result;
};
...
describe("the copy behaves exactly as v1 did", () => {
  it.each(CASES)("%s", (description, amount, accountKind, detail) => {
    ...
    expect(classify(statement, context)).toEqual(
      classifyCase(caseKey(description, amount, accountKind, detail)),
    );
  });
  ...
  it("agrees on history, which outranks the merchant guess", () => {
    ...
    expect(classify(statement, context)).toEqual(v1.history);
...
describe("the helpers agree too", () => {
  ...
  it.each(DESCRIPTIONS)("normalises %s identically", (description) => {
    const recorded = (v1.normalise as { d: string; r: unknown }[]).find(
      (entry) => entry.d === description,
    );
    expect(normaliseMerchant(description)).toEqual(recorded!.r);
  });

  it.each(DESCRIPTIONS)(
    "strips the channel from %s identically",
    (description) => {
      const recorded = (v1.strip as { d: string; r: unknown }[]).find(
        (entry) => entry.d === description,
      );
      expect(stripChannel(description)).toEqual(recorded!.r);
    },
  );
  ...
  it("builds the same history map", () => {
    ...
    expect(Array.from(mine.entries())).toEqual(v1.historyFrom);
