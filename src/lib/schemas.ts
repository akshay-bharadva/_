...
/** Bounds from the `watchlist_items` CHECK constraints. */
...
export const watchlistItemSchema = z.object({
  name: boundedRequiredString(WATCHLIST_LIMITS.NAME, "Name"),
  /**
   * Optional, because a bank mutual fund has a code no quote feed carries and
   * is still worth tracking. The pattern mirrors the column's CHECK — the
   * value is interpolated into a request path by the edge function, so a loose
   * bound here would be the app's problem, not the database's.
   */
  symbol: z
    .string()
    .trim()
    .max(WATCHLIST_LIMITS.SYMBOL, "Symbol is too long")
    .regex(/^[A-Za-z0-9.\-^=]+$/, "Use letters, digits, . - ^ = only")
    .optional()
    .nullable()
    .or(z.literal("")),
  kind: z.enum(["stock", "etf", "bond", "mutual_fund", "other"]),
  institution: boundedOptionalString(
    WATCHLIST_LIMITS.INSTITUTION,
    "Institution",
  ),
  currency: z
    .string()
    ...
    .nullable()
    .or(z.literal("")),
  // A holding you have not bought yet is zero, which is a real answer.
  quantity: z
    .number()
  ...
  target_price: z
    .number()
    .positive("A target price must be above zero")
    .max(MONEY_MAX_18_4, "Target price is too large")
    .optional()
    .nullable(),
  notes: boundedOptionalString(WATCHLIST_LIMITS.NOTES, "Notes"),
  sort_order: z.number().int().optional(),
});

export type WatchlistFormValues = z.infer<typeof watchlistItemSchema>;
