...
- **`src/features/`** — Owns feature-first UI and feature-specific behavior, for public and admin surfaces alike. Public/shared: home, about, contact, blog, updates, sections, github, admin-auth, admin-shell. Admin modules: dashboard, tasks, habits, learning, calendar, notes, whiteboard, finance, inventory, content, blog-admin, life-updates, navigation, assets, settings, security, focus. Each is a flat directory of kebab-case files with named exports (page components stay default-export). **`finance` is the one exception**, and deliberately: it is layered `ui/ → domain (ledger/, commitments/, budgets/, goals/, forecast/, fx/, import/) → money/`, because the arithmetic there has to be testable without React and the dependency direction has to be enforceable by eye. Depth is earned by that rule, not taken by default — a module whose logic is a handful of helpers stays flat. Must not reach into unrelated feature internals; cross-feature behavior should use documented shared contracts.
...
- **`src/styles/themes.css`** — Owns the 56 theme presets. It is intentionally raw/unlayered so runtime-applied theme classes are preserved. Do not move runtime theme definitions into a tree-shakeable Tailwind layer. The preset list must stay in sync with `THEME_PRESETS` in `src/lib/constants.ts`.

- **`src/styles/typography.css`** — Owns the 14 typography presets and the shared font stacks. Same raw/unlayered constraint as `themes.css`; imported after it, so it wins at equal specificity. Must stay in sync with `TYPOGRAPHY_PRESETS` in `src/lib/constants.ts`. (`themes.css` still contains a superseded legacy `.typo-*` block — edit typography presets here, not there.)
...
- Theme through tokens — use semantic theme classes and tokens so all 56 presets and custom themes continue to work. `chart-2` is the success accent and `chart-3` the warning accent; literal palette classes such as `text-green-600` or `bg-amber-500` do not move with the presets and must not be reintroduced.
